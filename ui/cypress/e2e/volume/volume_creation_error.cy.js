...
describe('volume creation fails', () => {
  it('should display a failed banner if the K8s API is not available', () => {
    cy.intercept('POST', '/api/kubernetes/apis/storage.metalk8s.scality.com/v1alpha1/volumes', {
    ...
    // V
    cy.findByText(/Volume test-volume-sparse creation has failed./i).should('exist');
  });
  ...
  it('should display failed status', () => {
    ...
    // V
    cy.findByText(/Volume test-volume-sparse is being created./i).should('exist');
