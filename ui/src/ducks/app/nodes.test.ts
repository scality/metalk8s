...
describe('`fetchNodes` saga', () => {
  ...
  test.each([
    ...
    const expectedNode = { ...defaultNodeForState, roles: expected.labels };
    expect(clone.next(apiResponse).value).toEqual(
      put({
        type: UPDATE_NODES,
        payload: {
...
describe.skip('`createNode` saga', () => {
  test('has the correct nominal flow', () => {
    ...
    expect(
      gen.next({
        whatever: 'nominal result',
      }).value,
    ).toEqual(call(fetchNodes));
    gen.next().value;
    const historyMock = {
      push: jest.fn(),
    };
    ...
    ).toEqual(call(historyMock.push, '/nodes'));
