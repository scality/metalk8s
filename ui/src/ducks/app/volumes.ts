...
import { call, delay, type Effect, put, select, takeLatest } from 'redux-saga/effects';
import { LVM_LOGICAL_VOLUME, RAW_BLOCK_DEVICE, REFRESH_TIMEOUT, SPARSE_LOOP_DEVICE } from '../../constants';
...
export const fetchCurrentVolumeObjectAction = (volumeName: string) => {
  return {
    type: FETCH_CURRENT_VOLUME_OBJECT,
    volumeName,
  };
};
...
type Volume = {
  ...
  storageClass: string;
  type: string;
  // TODO we might want to constraint it to 'sparseLoopDevice' | 'rawBlockDevice' | 'lvmLogicalVolume'
  size: string;
  vgName: string;
