...
import { FAKE_CONTROL_PLANE_IP, render } from './__TEST__/util';
import NodePartitionTable from './NodePartitionTable';

const server = setupServer(
  rest.get(`http://${FAKE_CONTROL_PLANE_IP}:8443/api/prometheus/api/v1/query_range`, (req, res, ctx) => {
    const result = {
      status: 'success',
      data: {
        resultType: 'matrix',
  ...
  rest.get(`http://${FAKE_CONTROL_PLANE_IP}:8443/api/prometheus/api/v1/query`, (req, res, ctx) => {
    const result = {
      status: 'success',
      data: {
        resultType: 'vector',
  ...
  rest.get(`http://${FAKE_CONTROL_PLANE_IP}:8443/api/loki/loki/api/v1/query_range`, (req, res, ctx) => {
    const result = {
      status: 'success',
      data: {
        resultType: 'streams',
...
describe('the system partition table', () => {
  ...
  test('displays the table', async () => {
    ...
    expect(screen.getByText(/Loading system partitions.../i)).toBeInTheDocument();
    // E
    await waitForElementToBeRemoved(() => screen.getByText(/Loading system partitions.../i));
    // Verify
  ...
  test('handles server error', async () => {
    ...
    // V
    expect(
      screen.getByText('An error occurred while loading the system partitions, please refresh the page.'),
    ).toBeInTheDocument();
  });
  afterAll(() => {
    server.close();
