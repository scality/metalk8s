...
def _download_deb_packages(releasever: str) -> types.TaskDict:
    ...
    mounts = [
        utils.bind_ro_mount(
            source=constants.ROOT/'packages'/'debian'/
                'common'/'download_packages.py',
            target=Path('/download_packages.py'),
        ),
        utils.bind_mount(
            source=constants.PKG_DEB_ROOT/releasever,
            target=Path('/repositories')
        ),
    ]
    dl_packages_callable = docker_command.DockerRun(
    ...
    return {
        ...
        'clean': [clean],
        'uptodate': [
            doit.tools.config_changed(_TO_DOWNLOAD_DEB_CONFIG[releasever])
        ],
        # Prevent Docker from polluting our output.
        'verbosity': 0,
    }
...
def _deb_package(
    name: str, releasever: str, sources: Path
) -> targets.DEBPackage:
    try:
...
def _deb_repository(
    name: str, releasever: str,
    packages: Optional[Sequence[targets.DEBPackage]]=None
...
_DEB_TO_BUILD_PKG_NAMES : Dict[str, List[str]] = \
    _list_packages_to_build(DEB_TO_BUILD)


# Store these versions in a dict to use with doit.tools.config_changed
_TO_DOWNLOAD_DEB_CONFIG: Dict[str, Dict[str, Optional[str]]] = \
    _list_packages_to_download(
        versions.DEB_PACKAGES, _DEB_TO_BUILD_PKG_NAMES,
    )
