...
class DEBRepository(Repository):
    ...
    def __init__(
        self, basename: str, name: str, builder: image.ContainerImage, **kwargs: Any
    ):
        super().__init__(basename, name, builder, constants.REPO_DEB_ROOT, **kwargs)
        task = self.basic_task
    ...
    def build_repo(self) -> types.TaskDict:
        ...
        actions.append(self._buildrepo_action())
        targets = [
            Path(self.rootdir, "dists/bionic", self.fullname, "binary-amd64/Packages")
        ]
        targets.extend(mkdir["targets"])
