...
class RemoteImage(image.ContainerImage):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        self._targets = list(self.filepaths)
    ...
    def tar_filepath(self) -> Path:
        """Path of the tar-file that would be created."""
    ...
    def image_filepath(self) -> Path:
        """Path to a sentinel file for the image that would be created."""
        return self.dirname/'manifest.json'

    @property
    def filepaths(self) -> Iterator[Path]:
        """Iterate files generated by this task."""
        if self._use_tar:
            yield self.tar_filepath
        if not self._use_tar or not self._tar_only:
            yield self.image_filepath
