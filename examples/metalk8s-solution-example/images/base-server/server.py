...
def _parse_timezone(timezone):
    error_msg = ('Invalid timezone (must match format "UTC[±HH:MM])": {}').format(
        timezone
    )
    match = re.match(r"UTC(?:(?P<sign>[\+\-])(?P<value>\d{2}:\d{2}))?", timezone)
    if match is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clock", dest="timezone", help="timezone to use for /clock")
    parser.add_argument("--host", default="", help="the host to listen on")
