...
def select_package_version(
    ...
    for candidate in package.versions:
        if version in candidate.version:
            return candidate
    raise ValueError(
        "package {} not found in version {}: {}".format(
...
def select_package_origin(package: apt.package.Version) -> str:
    ...
    raise Exception(
        "no trusted origin found for package {}".format(
            pathlib.Path(package.filename).name
        )
    )
...
# repositories…).
def fetch_binary(package: apt.package.Version, destdir: pathlib.Path) -> pathlib.Path:
    ...
    if destfile.stat().st_size != package.size:
        raise Exception("Failed to download package {}: size mismatch".format(filename))
    # Check package checksum.
    with open(destfile) as fp:
        if apt_pkg.sha256sum(fp) != package.sha256:
            raise Exception(
                "Failed to download package {}: checksum failed".format(filename)
            )
    return destfile
    ...
    repositories = [
        ...
        },
        {
            ...
            "key": urllib.parse.urljoin(
                ...
                "18.04/amd64/archive/{}/SALTSTACK-GPG-KEY.pub".format(salt_version),
            ),
