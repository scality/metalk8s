...
{%- set object_list = [
  ...
  ('v1', 'ServiceAccount', ingress_name ~ '-backend'),
...
# Default backend name is different between `nginx-ingress` and `ingress-nginx`
# And also with `ingress-nginx` a ConfigMap is also created
{%- if ingress_name == 'ingress-nginx' %}
  {%- do object_list.extend([
    ('v1', 'Service', ingress_name ~ '-defaultbackend'),
    ('apps/v1', 'Deployment', ingress_name ~ '-defaultbackend'),
    ('v1', 'ConfigMap', ingress_name ~ '-controller')
  ]) %}
{%- else %}
  {%- do object_list.extend([
    ('v1', 'Service', ingress_name ~ '-default-backend'),
    ('apps/v1', 'Deployment', ingress_name ~ '-default-backend'),
  ]) %}
{%- endif %}
