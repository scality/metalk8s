...
def saltutil_runner(salt_mock: SaltMock, method: str, **kwargs: Any) -> Any:
    ...
    if method == "manage.up":
        # Assume all names in pillar.metalk8s.nodes are available minions
        nodes_pillar = salt_mock._pillar.get("metalk8s", {}).get("nodes")
        if nodes_pillar is None:
            # No pillar, we assume only the current minion is available
            return [salt_mock._grains["id"]]
        return list(nodes_pillar.keys())
