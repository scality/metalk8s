...
YAML_TESTS_FILE = os.path.join(
    ...
    "files",
    "test_metalk8s_package_manager_yum.yaml",
)
...
class Metalk8sPackageManagerYumTestCase(TestCase, mixins.LoaderModuleMockMixin):
    ...
    loader_module_globals = {"__grains__": {"os_family": "RedHat"}}
    ...
    def test_virtual_success(self):
        ...
        self.assertEqual(
            metalk8s_package_manager_yum.__virtual__(), "metalk8s_package_manager"
        )
    ...
    def test_virtual_fail_import(self):
        ...
        """
        with patch.dict(
            metalk8s_package_manager_yum.__grains__, {"os_family": "Debian"}
        ):
            reload(metalk8s_package_manager_yum)
            self.assertEqual(metalk8s_package_manager_yum.__virtual__(), False)
    ...
    def test_list_dependents(self, repoquery_output, result, **kwargs):
        ...
        if repoquery_output is None:
            repoquery_cmd_kwargs = {"retcode": 1, "stderr": "An error has occured"}
        else:
            repoquery_cmd_kwargs = {"stdout": repoquery_output}
        ...
        with patch.dict(
            metalk8s_package_manager_yum.__salt__, {"cmd.run_all": repoquery_cmd_mock}
        ):
            self.assertEqual(
                metalk8s_package_manager_yum._list_dependents(
                    "my_package", "1.2.3", **kwargs
                ),
                result,
            )
            repoquery_cmd_mock.assert_called_once()
            self.assertEqual(repoquery_cmd_mock.call_args[0][0][0], "repoquery")
            if "fromrepo" in kwargs:
                self.assertIn(kwargs["fromrepo"], repoquery_cmd_mock.call_args[0][0])

    @utils.parameterized_from_cases(YAML_TESTS_CASES["list_pkg_dependents"])
    def test_list_pkg_dependents(
        self, result, list_dependents=None, rpm_qa_outputs=None, **kwargs
    ):
        """
        ...
        salt_dict = {"cmd.run_all": MagicMock(side_effect=_rpm_qa_cmd)}
        ...
        with patch.dict(metalk8s_package_manager_yum.__salt__, salt_dict), patch(
            ...
            self.assertEqual(
                metalk8s_package_manager_yum.list_pkg_dependents(
                    kwargs.pop("name", "my_package"), **kwargs
                ),
                result,
            )

    @utils.parameterized_from_cases(YAML_TESTS_CASES["check_pkg_availability"])
    def test_check_pkg_availability(
        self, pkgs_info, exclude=None, raise_msg=None, yum_install_retcode=0
    ):
        """
        ...
        salt_dict = {"cmd.run_all": cmd_run_mock}
        ...
        with patch.dict(metalk8s_package_manager_yum.__salt__, salt_dict):
            if raise_msg:
                ...
                metalk8s_package_manager_yum.check_pkg_availability(pkgs_info, exclude)
            if exclude:
                if isinstance(exclude, list):
                    exclude = ",".join(exclude)
                self.assertIn(
                    "exclude={}".format(exclude), cmd_run_mock.call_args[0][0]
                )
