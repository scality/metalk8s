...
YAML_TESTS_FILE = os.path.join(
    ...
    "files", "test_metalk8s_monitoring.yaml"
)
...
class Metalk8sMonitoringTestCase(TestCase, mixins.LoaderModuleMockMixin):
    ...
    def test_alertmanager_api_helper(
        self, _id, endpoint, route, result, raises=False, called_with=None,
        request_raises=False, resp_body=None, resp_status=200,
        **kwargs
    ):
    ...
    def test_add_silence(self, _id, value, call_body, now_mock=None, **kwargs):
        ...
        with patch(
            ...
            self.assertEqual(
                metalk8s_monitoring.add_silence(value, **kwargs),
                silence_id
            )
    ...
    def test_get_silences(self, _id, response, result, state=None):
        ...
        with patch(
            ...
            self.assertEqual(
                metalk8s_monitoring.get_silences(state=state), result
            )
    ...
    def test_get_alerts(self, _id, response, result, state=None):
        ...
        with patch(
            ...
            self.assertEqual(
                metalk8s_monitoring.get_alerts(state=state), result
            )
