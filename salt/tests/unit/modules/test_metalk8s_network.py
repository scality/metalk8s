...
class Metalk8sNetworkTestCase(TestCase, mixins.LoaderModuleMockMixin):
    ...
    @parameterized.expand([("192.168.1.10"), ("")])
    def test_get_control_plane_ingress_ip(self, result):
        """
        Tests the return of `get_control_plane_ingress_ip` function
        """
        pillar_get = MagicMock(return_value=result)
        with patch.dict(metalk8s_network.__salt__, {"pillar.get": pillar_get}):
            self.assertEqual(metalk8s_network.get_control_plane_ingress_ip(), result)

    @parameterized.expand([("https://192.168.1.10:8443"), ("")])
    def test_get_control_plane_ingress_endpoint(self, result):
        """
