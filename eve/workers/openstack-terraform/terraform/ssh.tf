...
resource "null_resource" "copy_bastion_pub_to_bootstrap" {
  ...
  provisioner "local-exec" {
    command = "scp -3 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i \"~/.ssh/terraform\" centos@${local.bastion_ip}:/home/centos/.ssh/bastion.pub centos@${local.bootstrap_ip}:/tmp/bastion.pub"
  }
  provisioner "remote-exec" {
    ...
    inline  = [
      "cat /tmp/bastion.pub >> /home/centos/.ssh/authorized_keys"
    ]
  }
}
...
resource "null_resource" "copy_bastion_pub_to_nodes" {
  ...
  provisioner "local-exec" {
    command = "scp -3 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i \"~/.ssh/terraform\" centos@${local.bastion_ip}:/home/centos/.ssh/bastion.pub centos@${local.nodes[count.index].ip}:/tmp/bastion.pub"
  }
  provisioner "remote-exec" {
    connection {
