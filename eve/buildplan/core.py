...
class Stage:
    ...
    def __hash__(self):
        return hash(yaml_print.dump(self.dump()))
...
class Step(metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, name, halt_on_failure=None, always_run=None,
    ):
        self._name = name
...
class ShellCommand(Step):
    def __init__(
        ...
        **kwargs
        ...
        self._sigterm_time = sigterm_time
        self._use_pty = use_pty

    command = property(operator.attrgetter("_command"))
    env = property(operator.attrgetter("_env"))
    workdir = property(operator.attrgetter("_workdir"))
    sigterm_time = property(operator.attrgetter("_sigterm_time"))
    use_pty = property(operator.attrgetter("_use_pty"))

    def dump_arguments(self):
        args = [("command", self.command)]
        if self.env is not None:
            args.append(("env", collections.OrderedDict(self.env.items())))
        if self.workdir is not None:
            args.append(("workdir", self.workdir))
        if self.use_pty is not None:
            args.append(("usePTY", self.use_pty))
        if self.sigterm_time is not None:
            args.append(("sigtermTime", self.sigterm_time))
        return args


class Upload(Step):
    def __init__(self, name, source, urls=None, **kwargs):
        super(Upload, self).__init__(name, **kwargs)
        self._source = source
        self._urls = urls

    source = property(operator.attrgetter("_source"))
    urls = property(operator.attrgetter("_urls"))

    def dump_arguments(self):
        args = [("source", self.source)]
        if self.urls is not None:
            args.append(("urls", self.urls))
        return args
