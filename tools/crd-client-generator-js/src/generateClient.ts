...
export function generateClient(crdFile: string, destinationFile: string, prefix?: string) {
  ...
  const versions = crdSpec.apiVersion === 'apiextensions.k8s.io/v1' ? crdSpec.spec.versions.map(version => version.name) : [crdSpec.spec.version];
  const jsonSchemas = crdSpec.apiVersion === 'apiextensions.k8s.io/v1' ? crdSpec.spec.versions.reduce((aggregatedJsonSchemas, version) => ({...aggregatedJsonSchemas, [version.name]: version.schema.openAPIV3Schema}), {}) : {[crdSpec.spec.version]: crdSpec.spec.validation.openAPIV3Schema};
  ...
  const versionsClients = versions.map(version => {
    ...
    const typePrefix = (prefix || '') + version.replace(/^\w/, (c) => c.toUpperCase());
  ...
  return prettier.format(generateClientHeader + versionsClients.join('\n'), prettierOptions );
}
